...
-- batting pitching and people mostly


-- 1. Find all players in the database who played at Vanderbilt University. (65 rows)
...
WITH vandy AS (
	SELECT DISTINCT(pp.playerid), pp.namefirst, pp.namelast, s.schoolname
	FROM people pp
	INNER JOIN collegeplaying cp
	ON pp.playerid = cp.playerid
...
dollars AS (
	SELECT DISTINCT pp.playerid, ss.salary 
	FROM people pp
	INNER JOIN salaries ss
	ON pp.playerid = ss.playerid
)
...
-- Do you see any trends? (Hint: For this question, 
-- you might find it helpful to look at the generate_series function 
...
-- 6. Which managers have won the TSN Manager of the Year award in both the 
...
-- Note that a player being inducted into the hall of fame 
-- is indicated by a 'Y' in the inducted column of the halloffame table.
...
-- and who hit at least one home run in 2016. Report the players' 
